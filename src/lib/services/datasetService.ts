 ...
 * @param annotationType The type of annotation to perform (e.g., "bounding_box", "polygon").
 * @returns A promise that resolves to the annotation results.
 * @throws Will throw an error if the backend call fails or returns invalid data.
 */
export async function performAutoAnnotation(
    ...
    console.log(
        `Service: Performing auto-annotation (${annotationType}) for path: ${path}, page: ${page}, pageSize: ${pageSize}`
    );

    if (!path || !path.trim()) {
        throw new Error("Directory path cannot be empty for auto-annotation.");
    }
    if (!annotationType || !annotationType.trim()) {
        throw new Error("Annotation type cannot be empty for auto-annotation.");
    }

    try {
        const resultStr = await invoke("auto_annotate_images", {
            path: path,
            page: page, // Pass these through if backend expects them
            pageSize: pageSize,

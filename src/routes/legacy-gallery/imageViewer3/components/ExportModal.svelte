<script lang="ts">
    let {
        ...
        isOpen: boolean;
        settings: { outputDir: string; trainRatio: number; valRatio: number; testRatio: number; shapeType: string };
        loading: boolean;
        error: string;
        success: string;
        datasetSummary: any;
        ...
        onclose?: () => void;
        onselectdir?: () => void;
        ontoggleexclusion?: (label: string) => void;
        onexport?: () => void;
    } = $props();
</script>
    ...
    >
        ...
        >
            ...
            <div class="px-6 py-4 overflow-y-auto">
                {#if success}
                    <div class="bg-green-50 text-green-700 p-3 rounded-md mb-4">{success}</div>
                {/if}
                {#if error}
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="sourceDirInput" value={sourceDir} readonly class="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-500" />
                </div>

                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1" for="outputDirInput">Output Directory</label>
                    <div class="flex gap-2">
                        <input type="text" id="outputDirInput" bind:value={settings.outputDir} readonly class="flex-1 px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-500" />
                        <button onclick={() => onselectdir?.()} class="px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-md border border-gray-300">Browse...</button>
                    </div>
                </div>

                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1" for="shapeTypeSelect">Shape Type</label>
                    <select id="shapeTypeSelect" bind:value={settings.shapeType} class="w-full px-3 py-2 border border-gray-300 rounded-md">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" id="trainRatioInput" bind:value={settings.trainRatio} min="0" max="1" step="0.1" class="w-full px-3 py-2 border border-gray-300 rounded-md" />
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1" for="valRatioInput">Validation Ratio</label>
                    <input type="number" id="valRatioInput" bind:value={settings.valRatio} min="0" max="1" step="0.1" class="w-full px-3 py-2 border border-gray-300 rounded-md" />
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1" for="testRatioInput">Test Ratio</label>
                    <input type="number" id="testRatioInput" bind:value={settings.testRatio} min="0" max="1" step="0.1" class="w-full px-3 py-2 border border-gray-300 rounded-md" />
                </div>

                <div class="mb-4">
                    <span class="block text-gray-700 mb-2 font-medium">Labels to Export</span>
                        ...
                        <div class="flex flex-wrap gap-2">
                            {#each Object.entries(datasetSummary.label_counts) as [label, count]}
                                <button
                                    ...
                                    onclick={() => ontoggleexclusion?.(label)}
                                >
                                    {label} ({count})
                                </button>
                            {/each}
            ...
            <div class="px-6 py-4 border-t bg-gray-50 flex justify-end space-x-2 mt-auto">
                <button class="px-4 py-2 border rounded-md hover:bg-gray-100" onclick={() => onclose?.()} disabled={loading}>Cancel</button>
                <button
                    class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md disabled:opacity-50 flex items-center"
                    onclick={() => onexport?.()}
                    disabled={loading || !settings.outputDir || !datasetSummary?.label_counts}
                >
                    {#if loading}

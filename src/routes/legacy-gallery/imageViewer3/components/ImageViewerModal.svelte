<script lang="ts">
    ...
    let { image, annotationType, onclose }: {
        image: any;
    ...
    function drawAnnotations() {
        ...
        const imageElement = document.getElementById("selected-image") as HTMLImageElement;
        const canvasElement = document.getElementById("annotation-canvas") as HTMLCanvasElement;
...
<div class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
    <div class="relative max-w-6xl w-full bg-white rounded-lg shadow-xl overflow-hidden">
        <div class="flex justify-between items-center p-4 border-b">
            <h3 class="text-lg font-medium text-gray-800 truncate">{image.name}</h3>
            <button onclick={() => onclose?.()} class="text-gray-400 hover:text-gray-500">✕</button>
        </div>
        <div class="p-4 flex flex-col items-center justify-center max-h-[calc(100vh-8rem)] overflow-auto">
            <div class="relative">
                <img id="selected-image" src={image.previewUrl} alt={image.name} class="max-w-full max-h-[calc(100vh-12rem)] object-contain" />
            ...
            {#if image.annotations && image.annotations.length > 0}
                <div class="mt-4 p-3 bg-green-50 rounded-md border border-green-100 max-w-xl w-full">
                    <h4 class="text-md font-medium text-green-800 mb-2">
                        {annotationType === "bounding_box" ? "Bounding Box" : "Polygon"} Annotations
                    </h4>
                    <ul class="space-y-1">
                        {#each image.annotations as annotation}
                            <li class="flex justify-between text-sm">
                                <span class="text-gray-700">{annotation.label}</span>
                                <span class="text-gray-500 font-medium">{annotation.shape_type}</span>
                            </li>
                        {/each}
                    </ul>
                </div>
        ...
        <div class="p-4 border-t bg-gray-50">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {#if image.size}
                    <div><span class="text-sm text-gray-500">Size:</span><span class="text-sm font-medium ml-1">{formatFileSize(image.size)}</span></div>
                {/if}
                {#if image.dimensions}
                    <div><span class="text-sm text-gray-500">Dimensions:</span><span class="text-sm font-medium ml-1">{image.dimensions.width} × {image.dimensions.height}</span></div>
                {/if}
                {#if image.created}
                    <div><span class="text-sm text-gray-500">Created:</span><span class="text-sm font-medium ml-1">{new Date(image.created).toLocaleString()}</span></div>
                {/if}
            </div>
        </div>
    </div>

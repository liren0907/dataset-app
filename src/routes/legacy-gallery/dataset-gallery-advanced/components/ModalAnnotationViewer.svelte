<script lang="ts">
    ...
    $: {
        if (showModal && selectedImage && !isInitializing) {
            ...
            if (!hasTriggeredInit || (hasTriggeredInit && !isInitialized)) {
                ...
                console.log('⏳ Using delayed initialization to ensure DOM readiness');
                setTimeout(() => {
    ...
    async function ensureKonvaContainerReady(maxRetries: number = 15, baseDelay: number = 100): Promise<void> {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            if (!container) {
                console.log(`🔍 Attempt ${attempt}: Trying to find container with querySelector...`);
                container = document.querySelector('[aria-label*="Interactive annotation editor"]') as HTMLDivElement;
                if (container) {
                    console.log(`✅ Found container via querySelector on attempt ${attempt}`);
            ...
            if (container) {
                console.log(`🔍 Attempt ${attempt}: Container exists, checking DOM attachment...`);

                // Additional verification: check if element is in document
                if (document.contains(container)) {
                    console.log(`🔍 Attempt ${attempt}: Container is in DOM, checking dimensions...`);

                    // Final check: ensure element has dimensions (fully rendered)
                    const rect = container.getBoundingClientRect();
                    if (rect.width > 0 && rect.height > 0) {
                        console.log(`✅ Konva container ready on attempt ${attempt} (${rect.width}x${rect.height})`);
                        return;
                    } else {
                        console.log(`⏳ Container exists but has zero dimensions (${rect.width}x${rect.height})`);
                    }
                } else {
            ...
            console.log(`⏳ Waiting ${delay}ms before next attempt (${attempt}/${maxRetries})...`);
        ...
        console.log('🔄 Final fallback: Attempting manual container discovery...');
        const manualContainer = document.querySelector('[aria-label*="Interactive annotation editor"]') as HTMLDivElement;
        ...
        if (manualContainer) {
            ...
            if (rect.width > 0 && rect.height > 0) {
                console.log(`🎉 Final fallback successful! Container ready (${rect.width}x${rect.height})`);
                return;
            } else {
                console.log('⚠️ Manual container found but has zero dimensions, will still proceed');
                return;
            }
        }
        ...
        console.error('📊 Debug info:', {
            ...
            documentContains: konvaContainer ? document.contains(konvaContainer) : false,
            querySelectorFound: !!document.querySelector('[aria-label*="Interactive annotation editor"]'),
        ...
        throw new Error(`Konva container not ready after ${maxRetries} attempts. This usually indicates a DOM timing issue.`);
    }
    ...
    async function initializeModal(): Promise<void> {
        ...
        if (isInitializing || (hasTriggeredInit && isInitialized)) {
            console.log('⚠️ Initialization blocked - isInitializing:', isInitializing, 'isInitialized:', isInitialized, 'hasTriggeredInit:', hasTriggeredInit);
            return;
        }

        isInitializing = true; // ← NEW: Prevent duplicate calls
        console.log('🚀 STARTING modal initialization for:', selectedImage.name);
        ...
        if (!konvaManager) {
            throw new Error('KonvaManager not initialized - check konvaService import');
        }

        try {
            isLoading = true;
            console.log('🚀 Initializing modal annotation viewer for:', selectedImage.name);
            ...
            if (annotatedImageData && konvaContainer) {
                ...
                throw new Error('Missing required data for KonvaJS initialization');
            }
            ...
            if (isInitialized) {
                ...
                console.log('✅ MODAL INITIALIZATION COMPLETED for:', selectedImage?.name);
            } else {
                // Reset hasTriggeredInit if initialization failed, allowing retry
                hasTriggeredInit = false;
                console.log('❌ MODAL INITIALIZATION FAILED for:', selectedImage?.name);
            }
        }
    }
    ...
    async function loadAnnotationMetadata(): Promise<void> {
        ...
        try {
            ...
            console.log('🚀 Invoking Tauri command: generate_single_annotated_preview');
            const result = await invoke("generate_single_annotated_preview", {
                imagePath: selectedImage.path
            }) as string;
            console.log('✅ Tauri command completed, result length:', result.length);

            const data = JSON.parse(result);

            if (data.annotation_metadata) {
                // Parse annotations from LabelMe format
                const parsedAnnotations = parseAnnotationsFromLabelMe(data.annotation_metadata);

                // Use original image with parsed annotations
                annotatedImageData = {
                    id: `modal_${selectedImage.path}`,
                    path: selectedImage.path,
                    previewUrl: selectedImage.previewUrl || convertFileSrc(selectedImage.path),
                    name: selectedImage.name,
                    annotations: parsedAnnotations
                };

                console.log('✅ Backend metadata loaded:', parsedAnnotations.length, 'annotations from LabelMe format');
            } else {
                throw new Error('No annotation metadata returned from backend');
            }

        } catch (error) {
            console.warn('⚠️ Backend preview generation failed, using fallback:', error);
    ...
    async function initializeKonvaViewer(): Promise<void> {
        ...
        console.log('Konva container dimensions:', konvaContainer?.clientWidth, 'x', konvaContainer?.clientHeight);

        try {
            // Load original image with parsed annotations from backend metadata
            await konvaManager.loadImageWithAnnotations(annotatedImageData, (scale, offsetX, offsetY) => {
                console.log('📋 Drawing annotations from backend metadata');
                konvaManager.drawAnnotations(annotatedImageData!.annotations, scale, offsetX, offsetY);
            });

            isInitialized = true;
            console.log('✅ Modal annotation viewer initialized successfully - isInitialized set to true');
        } catch (konvaError) {
    ...
    async function fallbackInitialization(): Promise<void> {
        ...
        try {
            ...
            console.log('✅ Fallback: Konva container is ready:', konvaContainer);
        } catch (error) {
        ...
        try {
            ...
            console.log('Container dimensions:', konvaContainer?.clientWidth, 'x', konvaContainer?.clientHeight);
            ...
            console.log('🔄 Fallback: Loading image with annotations:', fallbackImageData.annotations.length);

            // Load image with progressive annotation rendering
            await konvaManager.loadImageWithAnnotations(fallbackImageData, (scale, offsetX, offsetY) => {
                console.log('📋 Drawing annotations progressively');
                konvaManager.drawAnnotations(fallbackImageData.annotations, scale, offsetX, offsetY);
            });
    ...
    function cleanupModal(): void {
        ...
        isLoading = false;
        console.log('🧹 Modal cleanup completed - all flags reset, ready for next open');
    }
    ...
    function handleKeydown(event: KeyboardEvent): void {
        ...
        const handledKeys = ['Delete', 'Backspace', 'a', 'A', 'Escape', '=', '+', '-', '_', '0', 'r', 'R', 's', 'S'];

        if (handledKeys.includes(event.key) || (event.ctrlKey && event.key.toLowerCase() === 'a') || (event.ctrlKey && event.key.toLowerCase() === 's')) {
    ...
    // Control handlers
    function handleZoomIn(): void { konvaManager?.zoomIn(); }
    function handleZoomOut(): void { konvaManager?.zoomOut(); }
    function handleResetZoom(): void { konvaManager?.resetZoom(); }
    function handleFitToScreen(): void { konvaManager?.fitToScreen(); }
    function handleSelectAll(): void { konvaManager?.selectAllAnnotations(); }
    function handleDeselect(): void { konvaManager?.deselectAnnotation(); }
...
{#if showModal}
    <!-- Modal Overlay -->
    {console.log('🎨 Modal HTML is rendering, showModal:', showModal, 'selectedImage:', !!selectedImage)}
    <div

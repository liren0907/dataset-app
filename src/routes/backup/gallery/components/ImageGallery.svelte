        ...
        {#if viewMode === "grid"}
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {#each images as image, i (image.path)}
                    <button
                        type="button"
                        class="card bg-base-100 shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer border border-base-300 hover:-translate-y-0.5"
        ...
        {:else}
            ...
            <div class="space-y-4">
                {#each images as image, i (image.path)}
                    <button
                        type="button"
                        class="card card-side bg-base-100 shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer border border-base-300 w-full hover:-translate-y-0.5"
                        on:click={() => handleImageClick(image, i)}
                        on:mouseenter={() => handleImageHover(image)}
                        on:mouseleave={handleImageLeave}
                        aria-label={`View details for ${image.name}`}

...
impl YoloExporter {
    ...
    fn generate_class_map(&self, json_files: &[PathBuf]) -> Result<HashMap<String, usize>, Box<dyn Error>> {
    ...
    fn save_dataset_yaml(&self, output_dir: &Path, class_map: &HashMap<String, usize>) -> io::Result<()> {
        let yaml_path = output_dir.join("dataset.yaml");
        let mut file = BufWriter::new(File::create(yaml_path)?);
        
        // Create sorted classes vector
        let mut classes: Vec<(String, usize)> = class_map.iter().map(|(k, v)| (k.clone(), *v)).collect();
    ...
    fn convert_file(
        ...
        if !source_image.exists() {
            ...
            if !fallback_source_image.exists() {
             ...
             fs::copy(fallback_source_image, images_dir.join(split).join(source_image.file_name().unwrap()))?;
        } else {
             // Use original path
             fs::copy(&source_image, images_dir.join(split).join(source_image.file_name().unwrap()))?;
        }
        ...
        if let Some(shapes) = json["shapes"].as_array() {
            for shape in shapes { // Iterate through each shape in the JSON
                ...
                if let (Some(label), Some(shape_type), Some(points)) = (label_opt, shape_type_opt, points_opt) {
                    // --- Filtering Logic --- 
                    ...
                    if self.shape == "bounding_box" {
                        ...
                        if shape_type == "rectangle" || shape_type == "polygon" || shape_type == "bounding_box" {
                            ...
                            for point_val in points {
                                if let (Some(x), Some(y)) = (point_val[0].as_f64(), point_val[1].as_f64()) {
                    ...
                    // Polygon Output:
                        ...
                        if shape_type == "polygon" {
                            ...
                            for point_val in points {
                                if let (Some(x), Some(y)) = (point_val[0].as_f64(), point_val[1].as_f64()) {
                                    normalized_points.push((x as f32 / img_width, y as f32 / img_height));
                                }
                            }
...
pub fn export_to_yolo_new(
    ...
    if let Err(e) = std::fs::create_dir_all(&final_output_path) {
        return Err(format!("Failed to create output directory {}: {}", final_output_path.display(), e));
    }
    
    // Create exporter
    let exporter = YoloExporter::new(
        &source_dir,
        final_output_path.to_str().ok_or("Invalid final output path string")?,
        train_ratio,
        val_ratio,
        test_ratio,
        &shape,
    ...
    match exporter.export() {
        Ok(_) => Ok("Dataset exported to YOLO format successfully (using new function)".to_string()),
        Err(e) => Err(format!("Failed to export to YOLO format (using new function): {}", e))
    }
}

// Backup code for the export_to_yolo_new function modification
// #[tauri::command]
// pub fn export_to_yolo_new(
//     source_dir: String, 
//     output_dir: String, 
...
//     // Create exporter
//     // Assuming YoloExporter::new takes these arguments based on main.rs usage
//     let exporter = YoloExporter::new(
//         &source_dir,
//         &output_dir,
//         train_ratio,
...
mod tests {
    ...
    #[test]
    fn test_yolo_exporter() {
        // Create temporary directories
        let source_dir = tempdir().unwrap();
        let output_dir = tempdir().unwrap();
        
        // Create a dummy JSON file
        let json_path = source_dir.path().join("test.json");
        let json_content = r#"{
            "imagePath": "test.jpg",
            "imageWidth": 800,
            "imageHeight": 600,
        ...
        let image_path = source_dir.path().join("test.jpg");
        fs::write(&image_path, vec![0; 100]).unwrap(); // Dummy image data
        ...
        let yaml_path = output_dir.path().join("dataset.yaml");
        assert!(yaml_path.exists(), "dataset.yaml not created");

...
pub fn draw_bounding_boxes(source_dir: &str, json_path: &str, output_dir: &str) -> Result<(), Box<dyn Error>> {
    // Read and parse the JSON file
    let json_content = fs::read_to_string(json_path)?;
    let json: Value = serde_json::from_str(&json_content)?;
    
    // Extract image path from the JSON
    let image_path = json["imagePath"].as_str().ok_or("Missing imagePath in JSON")?;
